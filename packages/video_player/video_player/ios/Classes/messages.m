...
- (NSDictionary *)toMap;
@end
@interface FLTResolutionMessage ()
+ (FLTResolutionMessage *)fromMap:(NSDictionary *)dict;
- (NSDictionary *)toMap;
@end
...
@implementation FLTResolutionMessage
+ (FLTResolutionMessage *)fromMap:(NSDictionary *)dict {
  FLTResolutionMessage *result = [[FLTResolutionMessage alloc] init];
  result.textureId = dict[@"textureId"];
  if ((NSNull *)result.textureId == [NSNull null]) {
    result.textureId = nil;
  }
...
void FLTVideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, id<FLTVideoPlayerApi> api) {
  ...
  {
    FlutterBasicMessageChannel *channel = [FlutterBasicMessageChannel
        messageChannelWithName:@"dev.flutter.pigeon.VideoPlayerApi.setPreferredResolution"
               binaryMessenger:binaryMessenger];
    if (api) {
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        FLTResolutionMessage *input = [FLTResolutionMessage fromMap:message];
        FlutterError *error;
        [api setPreferredResolution:input error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
